...
@WebServlet("/book_edit_action")
public class BookEditAction extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) { // 批判内容是否是普通文本(非文件内容)
					...
					if(StringUtils.isNullOrEmpty(value)) {
						...
						request.getRequestDispatcher("/book_edit").forward(request, response);
						return;
					}
			...
			Info info=new Info(null, bookName, author, publisher, price,
					bookCategory,bphoto);
			//把Info对象放入数据库
			int result=bookService.alterInfoById(info); 
			if(result != 0) {
				...
				request.setAttribute("message","修改书籍失败");
				request.getRequestDispatcher("/book_edit").forward(request,response);
				return;
			}  
		} catch (Exception e) {
